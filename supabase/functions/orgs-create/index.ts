...
async function createStytchOrg(name: string, slug: string) {
  ...
  if (!res.ok)
    throw new Error(`Stytch error: ${res.status} ${await res.text()}`);
  return res.json();
}
...
serve(async (req) => {
  try {
    ...
    const stytch_organization_id: string =
      st.organization?.organization_id ?? st.organization_id;
    if (!stytch_organization_id)
      throw new Error("Missing organization_id from Stytch");
